...
int enqNeighbors (struct image* i, Stack *s, struct position* p) {
	...
	if (count == 0) {
		...
		if (i->image[p->row][p->col] == 2) { /*single non-dead neighbor has been visited --> need to backtrack*/
			/*BACKTRACK PROCESS*/	
			while (((backCount = nonDeadNeighbors(i, p->row, p->col)) <= 1) && (i->image[p->row][p->col] == 2)) { /*while there is only one non-dead neighbor, and we've visited it --> set location to dead end and check the next non-dead neighbor, keep going till it splits somewhere or enqueued --> done backtracking*/
				i->image[p->row][p->col] = -1; /*if there is one or less nondead number --> set location to dead end and check the next single neighbor if there is one*/
				if (backCount == 1) {
					findNeighbor(i, p); /*find next single neighbor to check if that's a deadend*/
				} else {
					break; /*if no nondead neighbors --> done backtracking, break out of loop*/
				}
			}
